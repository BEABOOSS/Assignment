...
function setTimeClick(elem){
    ...
    let children = elem.parentNode.parentNode.childNodes;
...
let togglesState = false;
// added closedState to see if the timeInput was open or not 
let closedState = true;

window.addEventListener("click", (event) => {
    if(event.target.localName === "input") {
        if (togglesState && !closedState) {
            hideTimesBoxes();
            togglesState = false;
            closedState = false;
            return;
